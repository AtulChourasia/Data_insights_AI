...
async def check_file_token_limit(uploaded_file, max_tokens: int = 100000, model_name: str = "gpt-4") -> Tuple[bool, str, int, dict]:
    """
    Check if uploaded FastAPI file fits within token limit using accurate tiktoken estimation
    ...
    """
    try:
        # Get filename from FastAPI UploadFile
        filename = uploaded_file.filename
        if not filename:
            return False, "Cannot determine file type - no filename available", 0, {}
        ...
        if file_type == "csv":
            ...
            # Create BytesIO object from file content for Excel
            try:
        ...
        logger.error(f"Error in check_file_token_limit: {e}")
